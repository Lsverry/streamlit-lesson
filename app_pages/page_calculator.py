...
def calculator_body():
    ...
    with col1:
        num1 = st.number_input(label = "Introduce the first number")

    with col2:
        num2 = st.number_input(label = "Introduce the second number")

    with col3:
        operator = st.selectbox(label = "Select an operator", options = ["Add", "Subtraction", "Multiply", "Divide"])
